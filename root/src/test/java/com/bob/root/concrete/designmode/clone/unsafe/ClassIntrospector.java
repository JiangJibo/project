...
public class ClassIntrospector {
    ...
    private ObjectInfo introspect(final Object obj, final Field fld)
        ...
        // seen this object
        if (!isPrimitive) {
            if (m_visited.containsKey(obj)) { isRecursive = true; }
        ...
        if (!isRecursive && obj != null) {
            if (isObjectArray(type)) {
                ...
                for (final Object item : ar) { if (item != null) { root.addChild(introspect(item, null)); } }
            } else {
    ...
    private static List<Field> getAllFields(final Class type) {
        if (type.isPrimitive()) { return Collections.emptyList(); }
        Class cur = type;
    ...
    private static String getContents(final Object val, final Class type) {
        ...
        if (type.isArray()) {
            if (type == byte[].class) { return Arrays.toString((byte[])val); } else if (type == boolean[].class) {
